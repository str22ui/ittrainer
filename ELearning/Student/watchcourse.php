             ...
             if(isset($_GET['course_id'])){
              ...
              if($result->num_rows > 0){
               while($row = $result->fetch_assoc()){
                echo '<li class="nav-item border-bottom py-2" movieurl='.$row['lesson_link'].' style="cursor: pointer;">'. $row['lesson_name'];
                // Check if lesson_pdf exists
                if(!empty($row['lesson_pdf'])) {
                  echo '<a href="../lessonpdf/'.$row['lesson_pdf'].'" class="btn btn-secondary btn-sm ml-2" target="_blank">View PDF</a>';
                }
                echo '</li>';
               }
              }
